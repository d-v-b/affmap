...
class AxesDict(TypedDict, Generic[T_AxesIn, T_AxesOut]):
    ax_in : T_AxesIn
    ax_out: T_AxesOut 
...
class AffMap(BaseModel, Generic[T_AxesIn, T_AxesOut]):
    ...
    @property
    def axes(self) -> AxesDict[T_AxesIn, T_AxesOut]:
        return {
            'ax_in': tuple(self.translation.keys()),
            'ax_out': tuple(self.translation.keys())
            }

    def flatten(self, axes: AxesDict) -> tuple[float, ...]:
        out: tuple[float, ...] = ()
        for ax_o in axes['ax_out']:
            for ax_i in axes['ax_in']:
